...
def init_chat_db():
    ...
    # Мягкая миграция: если база старая и без is_read — добавить колонку
...
def handle_client(conn, addr):
    try:
        ...
        # ----------------- FRIEND REQUESTS -----------------
            ...
            cursor = conn_db.cursor()
            cursor.execute(
                "INSERT OR IGNORE INTO friends (user_login, friend_login) VALUES (?, ?)",
                (user, from_user)
            )
            cursor.execute(
                "INSERT OR IGNORE INTO friends (user_login, friend_login) VALUES (?, ?)",
        ...
        # ----------------- FRIENDS -----------------
            ...
            for r in rows:
                ...
                friends.append({
                    ...
                    "nickname": r[1],
                    "avatar": r[2],
                    "online": login_f in online_users
                })
        ...
        # ----------------- CHAT MESSAGES -----------------
            ...
            user_a = data.get("from_user")
            user_b = data.get("to_user")
            try:
                conn_db = sqlite3.connect(CHAT_DB_FILE)
                cursor = conn_db.cursor()
                cursor.execute("""
                    SELECT id, from_user, to_user, text, timestamp, is_read
                    FROM messages
            ...
            try:
                ...
                resp = {"status": "ok", "updated": changed}
            except Exception as e:
                resp = {"status": "error", "message": str(e)}
            finally:
                conn_db.close()
            ...
            try:
                ...
                resp = {"status": "ok", "counts": counts, "total": total}
            except Exception as e:
                resp = {"status": "error", "message": str(e)}
            finally:
                conn_db.close()
